...
\node[right] at (5,1.5) {Register value};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
% Declaration
\node[draw, rectangle, minimum width=6cm, fill=blue!15, draw=blue!60] (decl) at (0,3) {\texttt{int *voltage\_ptr;}};
\node[anchor=south west, color=blue!70] at ([yshift=0.2cm, xshift=1.2cm]decl.west) {\textbf{\tiny Type}};
\node[anchor=south, color=purple!70] at ([yshift=0.5cm]decl.center) {\textbf{\tiny * means pointer}};
\node[anchor=south east, color=teal!70] at ([yshift=0.23cm, xshift=-1.8cm]decl.east) {\textbf{\tiny Variable name}};

% Assignment
\node[draw, rectangle, minimum width=6cm, fill=teal!15, draw=teal!60] (assign) at (0,1.5) {\texttt{voltage\_ptr = \&voltage;}};
\node[anchor=south west, color=teal!70] at ([yshift=0.2cm, xshift=0.9cm]assign.west) {\textbf{\tiny Pointer variable}};
\node[anchor=south east, color=orange!70] at ([yshift=0.2cm, xshift=-0.8cm]assign.east) {\tiny \textbf{\& gets address}};

% Dereferencing
\node[draw, rectangle, minimum width=6cm, fill=purple!15, draw=purple!60] (deref) at (0,0) {\texttt{*voltage\_ptr = 5000;}};
\node[anchor=south west, color=purple!70] at ([yshift=0.2cm, xshift=1.2cm]deref.west) {\tiny \textbf{* accesses value}};
\node[anchor=south east, color=blue!70] at ([yshift=0.2cm, xshift=-1.0cm]deref.east) {\tiny \textbf{New value}};
\end{tikzpicture}
\end{center}
...
\textbf{Memory mantra:}
\begin{itemize}
    \item \textcolor{orange!70}{\texttt{\&}} = \textcolor{orange!70}{"address of"}
    \item \textcolor{purple!70}{\texttt{*}} = \textcolor{purple!70}{"value at"} (when used with pointer)
    \item \textcolor{purple!70}{\texttt{*}} = \textcolor{purple!70}{"pointer to"} (when declaring)
\end{itemize}
\end{frame}
