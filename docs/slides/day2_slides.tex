...
\hline
\small\texttt{\&\&} & \small AND & \footnotesize\texttt{voltage > 1.8 \&\& voltage < 3.6} \\
\small\texttt{||} & \small OR & \footnotesize\texttt{error\_code == 1 || error\_code == 2} \\
\small\texttt{!} & \small NOT & \footnotesize\texttt{!system\_ready} \\
\hline
\end{tabular}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\node[draw, rectangle, minimum width=6cm, minimum height=0.8cm] at (0,0)
{\footnotesize\texttt{\textcolor{teal}{int} \textcolor{purple}{check\_voltage}\textcolor{orange}{(float voltage, float min, float max)}}};

\node[above, teal] at (-4,0.6) {\tiny\textbf{Return Type}};
\draw[->, teal] (-4,0.7) -- (-4.2,0.2);

\node[above, purple] at (-2.5,0.63) {\tiny\textbf{Function Name}};
\draw[->, purple] (-2.5,0.7) -- (-2.7,0.2);

\node[above, orange] at (1.3,0.63) {\tiny\textbf{Parameters}};
\draw[->, orange] (1.3,0.7) -- (1.3,0.2);
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item \footnotesize Step through test sequences
    \item \footnotesize Inspect register values
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\begin{itemize}
    \item \footnotesize Find where tests fail
    \item \footnotesize Understand program flow
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\hline
\small\texttt{break function\_name} & \small Set breakpoint \\
\small\texttt{run} & \small Start program \\
\small\texttt{next} & \small Execute next line \\
\small\texttt{step} & \small Step into functions \\
\small\texttt{print variable} & \small Show variable value \\
\small\texttt{info locals} & \small Show all local variables \\
\small\texttt{backtrace} & \small Show call stack \\
\small\texttt{continue} & \small Resume execution \\
\hline
\end{tabular}
\end{center}

\begin{exampleblock}{Debugging Session}
\small\texttt{(gdb) break check\_voltage}\\
\small\texttt{(gdb) run}\\
\small\texttt{(gdb) print voltage}\\
\small\texttt{(gdb) next}
\end{exampleblock}
\end{frame}

\begin{frame}[fragile]
\frametitle{Validation Example: Register Monitor}
\begin{lstlisting}[language=C, basicstyle=\fontsize{4}{5}\selectfont\ttfamily]
#include <stdio.h>
