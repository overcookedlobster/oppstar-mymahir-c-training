...
\begin{tikzpicture}[
    ...
    box/.style={draw, rectangle, rounded corners=2pt, minimum width=1.8cm, minimum height=0.7cm, align=center, fill=gray!5, thick},
    process/.style={box, fill=blue!15, text=black, font=\tiny},
    file/.style={box, fill=green!10, text=black, font=\tiny},
    arrow/.style={->, thick, black!80, >=stealth}
]

% Process nodes in a row at the top
\node (cpp) [process] {Preprocessor};
\node (cc1) [process, right=of cpp] {Compiler};
\node (as) [process, right=of cc1] {Assembler};
\node (ld) [process, right=of as] {Linker};

% File nodes - source staggered left, intermediary files between processes
\node (source) [file, above=0.6cm of cpp, xshift=-0.35cm] {\textbf{Source}\\\texttt{.c}};
\node (preprocessed) [file, below=0.6cm of cpp, xshift=0.35cm] {\textbf{Preprocessed}\\\texttt{.i}};
\node (assembly) [file, below=0.6cm of cc1, xshift=0.35cm] {\textbf{Assembly}\\\texttt{.s}};
\node (object) [file, below=0.6cm of as, xshift=0.35cm] {\textbf{Object}\\\texttt{.o}};
\node (executable) [file, right=0.35cm of object] {\textbf{Executable}};

% Draw arrows
\draw [arrow] (source) -- (cpp);
\draw [arrow] (cpp) -- (preprocessed);
\draw [arrow] (preprocessed) -- (cc1);
\draw [arrow] (cc1) -- (assembly);
\draw [arrow] (assembly) -- (as);
\draw [arrow] (as) -- (object);
\draw [arrow] (object) -- (ld);
\draw [arrow] (ld) -- (executable);

\end{tikzpicture}
\end{center}

\textbf{GCC:} \texttt{gcc -Wall -g program.c -o program}
\begin{itemize}
    \item \texttt{-o} output filename
    \item \texttt{-Wall} show warnings
    \item \texttt{-g} debug symbols
\end{itemize}
\end{frame}
